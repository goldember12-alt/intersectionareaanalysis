...
OUT_DIR = OUTPUT_ROOT / "review/current/missing_hmms_ramp_terminal_context_refresh"
...
REQUIRED_INPUTS = [
    ...
    RECOVERY_DIR / "ramp_terminal_recovery_skipped_targets.csv",
    RECOVERY_DIR / "ramp_terminal_context_refresh_readiness.csv",
...
def _source_class_flags(source_class: pd.DataFrame) -> pd.DataFrame:
    ...
    for sid, group in source_class.groupby("stable_signal_id", dropna=False):
        ...
        row["ramp_mainline_contamination_flag"] = row["has_ramp_mainline_mixed_needs_subbranch_split"]
        row["grade_separated_mainline_exclusion_flag"] = row["has_grade_separated_mainline_exclude"]
        rows.append(row)
    return pd.DataFrame(rows)


def _build_base_bins() -> pd.DataFrame:
    bins = _read_csv(RECOVERY_DIR / "ramp_terminal_recovered_bins.csv")
    legs = _read_csv(RECOVERY_DIR / "ramp_terminal_recovered_leg_candidates.csv")
    signals = _read_csv(RECOVERY_DIR / "ramp_terminal_recovered_signal_summary.csv")
    overlap = _read_csv(RECOVERY_DIR / "ramp_terminal_overlap_dedup_review.csv")
    source_class = _read_csv(RECOVERY_DIR / "ramp_terminal_source_leg_classification.csv")
    flags = _source_class_flags(source_class)
    out = bins.merge(
        legs[["leg_candidate_id", "source_leg_class", "grade_or_mainline_risk_flag", "coverage_class"]].drop_duplicates("leg_candidate_id"),
        on="leg_candidate_id",
        how="left",
    )
    out = out.merge(
        signals[
            [
                ...
                "generated_bin_count",
                "excluded_mainline_source_rows",
                "mixed_ramp_mainline_source_rows",
            ]
        ],
        on="stable_signal_id",
        how="left",
    ...
    out["ramp_frontage_service_mainline_context"] = np.select(
        [
            _text(out, "source_leg_class").eq("signal_relevant_ramp_terminal_leg"),
            _text(out, "source_leg_class").eq("signal_relevant_frontage_or_service_road_leg"),
            _text(out, "source_leg_class").eq("ramp_mainline_mixed_needs_subbranch_split"),
            _flag(out, "grade_or_mainline_risk_flag"),
        ],
        ["ramp_terminal", "frontage_or_service", "ramp_mainline_mixed", "grade_or_mainline_risk"],
        default="surface_crossroad",
    )
...
def _attach_context(base: pd.DataFrame) -> pd.DataFrame:
    ...
    out["review_only_context_refresh_provenance"] = "missing_hmms_ramp_terminal_context_refresh"
    return out
...
def _signal_summary(detail: pd.DataFrame) -> pd.DataFrame:
    grouped = detail.groupby("stable_signal_id", dropna=False).agg(
        ...
        grade_mainline_contamination_risk=("grade_mainline_contamination_risk", "first"),
        ramp_mainline_contamination_flag=("ramp_mainline_contamination_flag", "first"),
        grade_separated_mainline_exclusion_flag=("grade_separated_mainline_exclusion_flag", "first"),
    ...
    grouped["overlap_or_ramp_mainline_risk"] = (
        ...
        | _flag(grouped, "grade_mainline_contamination_risk")
        | _flag(grouped, "ramp_mainline_contamination_flag")
        | _flag(grouped, "grade_separated_mainline_exclusion_flag")
    )
    grouped["clean_addition_candidate"] = grouped["speed_aadt_ready"] & ~grouped["overlap_or_ramp_mainline_risk"]
...
def _readiness_summary(signal_summary: pd.DataFrame) -> pd.DataFrame:
    metrics = [
        ("total_ramp_terminal_candidates", len(signal_summary)),
...
def _universe_projection(signal_summary: pd.DataFrame) -> pd.DataFrame:
    ...
    return pd.DataFrame(
        [
            {"metric": "current_clean_universe_before_ramp_terminal_refresh", "value": CURRENT_CLEAN_UNIVERSE},
            {"metric": "current_remaining_non_clean_before_ramp_terminal_refresh", "value": CURRENT_REMAINING_NONCLEAN},
            {"metric": "ramp_terminal_generated_candidates", "value": len(signal_summary)},
            {"metric": "ramp_terminal_context_ready_signals", "value": context_ready},
            {"metric": "ramp_terminal_clean_addition_candidates", "value": clean},
            {"metric": "ramp_terminal_risk_or_holdout_candidates", "value": risk},
...
def _missingness(detail: pd.DataFrame) -> pd.DataFrame:
    checks = {
        ...
        "ramp_mainline_contamination_flag": _flag(detail, "ramp_mainline_contamination_flag"),
        "grade_separated_mainline_exclusion_flag": _flag(detail, "grade_separated_mainline_exclusion_flag"),
    }
    return pd.DataFrame(
...
def _findings(signal_summary: pd.DataFrame, projection: pd.DataFrame) -> str:
    ...
    clean = int(signal_summary["clean_addition_candidate"].sum())
    risk = int(signal_summary["overlap_or_ramp_mainline_risk"].sum())
    ...
    return f"""# Ramp-Terminal Missing HMMS Context Refresh Findings

## Bounded Question

This review-only context refresh populates the 142 generated `signalized_ramp_terminal_recoverable` missing-HMMS candidates with route/measure identity, roadway context, RNS speed, and AADT v3/exposure context. It preserves ramp/mainline contamination and grade-separated mainline exclusion evidence. It does not promote signals, add access, assign crashes, calculate rates/models, or alter active outputs.

## Results

- Target ramp-terminal candidates: {len(signal_summary):,}
...
- Signals with overlap/duplicate/sibling/ramp-mainline risk: {risk:,}
- High-crash-relevance context-ready signals: {high_ready:,}
- Projected clean universe if clean candidates are accepted: {projected:,}

## Recommendation

Do not promote this branch yet. The next pass should integrate the context-ready ramp-terminal candidates into a review-only universe/risk decomposition, separating clean candidates from ramp/mainline contamination and same-corridor risk cases.
"""
...
def _qa(detail: pd.DataFrame, signal_summary: pd.DataFrame) -> pd.DataFrame:
    ...
    return pd.DataFrame(
        [
            ...
            {"check_name": "grade_separated_mainline_rows_excluded_or_held", "status": "passed", "observed": "grade_separated_mainline_exclusion_flag carried"},
            {"check_name": "ramp_mainline_contamination_flags_carried_forward", "status": "passed" if "ramp_mainline_contamination_flag" in detail.columns else "failed", "observed": f"{int(_flag(signal_summary, 'ramp_mainline_contamination_flag').sum())} flagged signals"},
...
def main() -> None:
    ...
    roadway_summary = _summary_table(detail, "ramp_frontage_service_mainline_context", "ramp_frontage_service_mainline_context")
    ...
    _write_csv(detail, "ramp_terminal_context_bin_detail.csv")
    _write_csv(signal_summary, "ramp_terminal_context_signal_summary.csv")
    _write_csv(route_summary, "ramp_terminal_route_measure_summary.csv")
    _write_csv(roadway_summary, "ramp_terminal_roadway_context_summary.csv")
    _write_csv(speed_summary, "ramp_terminal_speed_summary.csv")
    _write_csv(aadt_summary, "ramp_terminal_aadt_exposure_summary.csv")
    _write_csv(readiness_summary, "ramp_terminal_context_readiness_summary.csv")
    _write_csv(overlap_review, "ramp_terminal_existing_universe_overlap_review.csv")
    _write_csv(projection, "ramp_terminal_universe_expansion_projection.csv")
    _write_csv(missingness, "ramp_terminal_context_missingness.csv")
    _write_text(_findings(signal_summary, projection), "ramp_terminal_context_refresh_findings.md")
    _write_csv(qa, "ramp_terminal_context_refresh_qa.csv")

    manifest = {
        "created_utc": _now(),
        "script": "src.roadway_graph.missing_hmms_ramp_terminal_context_refresh",
        "review_only": True,
        ...
        "input_manifests": {
            ...
            "offset_complex_reclassification": _manifest_ref(OFFSET_COMPLEX_DIR / "offset_anchor_complex_risk_reclassification_manifest.json"),
            "stable_lineage": _manifest_ref(STABLE_DIR / "stable_lineage_generation_manifest.json"),
    ...
    _write_json(manifest, "ramp_terminal_context_refresh_manifest.json")
