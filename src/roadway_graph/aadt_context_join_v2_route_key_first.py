...
OUTPUT_DIR = Path("review/current/aadt_context_join_v2_route_key_first")
...
V1_AADT_CONTEXT_DIR = OUTPUT_ROOT / "review/current/aadt_context_join"
V1_AADT_SUMMARY_FILE = V1_AADT_CONTEXT_DIR / "aadt_context_join_summary.csv"
...
def normalize_route_name_v1(value: Any) -> str:
...
def normalize_route_name(value: Any) -> str:
    """Conservative route-key repair for AADT v2.

    This keeps route families explicit (US, SR, I) and strips punctuation,
    route words, whitespace, hyphens, and leading zeros without broad fuzzy
    matching or name-similarity matching.
    """
    text = str(value or "").upper().strip()
...
def _load_aadt() -> tuple[gpd.GeoDataFrame, pd.DataFrame]:
    ...
    for field in AADT_ROUTE_FIELDS:
        ...
        alias["aadt_route_name_normalized_v1"] = alias["aadt_route_name_raw"].map(normalize_route_name_v1)
        alias["aadt_route_key_v2"] = alias["aadt_route_name_raw"].map(normalize_route_name)
        alias["aadt_route_alias_field"] = field
        alias_frames.append(alias)
    aliases = pd.concat(alias_frames, ignore_index=True, sort=False)
    aliases = aliases.loc[aliases["aadt_route_key_v2"].ne("")].copy()
    aliases = aliases.drop_duplicates(["aadt_source_index", "aadt_route_key_v2"]).copy()
    return aadt, aliases
...
def _load_directional_context_bins() -> pd.DataFrame:
    ...
    bins["stable_route_name_normalized_v1"] = bins["stable_route_name_raw"].map(normalize_route_name_v1)
    bins["stable_route_key_v2"] = bins["stable_route_name_raw"].map(normalize_route_name)
...
def _load_base_bin_geometry(context_bins: pd.DataFrame) -> gpd.GeoDataFrame:
    ...
    stable = eligible[["source_bin_key", "base_segment_id", "stable_route_name_raw", "stable_route_name_normalized_v1", "stable_route_key_v2", "stable_edge_key", *[c for c in STABLE_ROUTE_FIELDS if c in eligible.columns]]].drop_duplicates(["source_bin_key", "base_segment_id"])
...
def _route_group_summary(routed_bins: gpd.GeoDataFrame, valid_aadt: gpd.GeoDataFrame) -> str:
    bin_groups = routed_bins.groupby("stable_route_key_v2", dropna=False).size().sort_values(ascending=False)
    aadt_groups = valid_aadt.groupby("aadt_route_key_v2", dropna=False).size()
    largest = []
...
def _route_bucket_summary(aadt: gpd.GeoDataFrame) -> pd.DataFrame:
    valid = aadt.loc[aadt["source_geometry_is_valid"] & aadt["aadt_route_key_v2"].ne("") & aadt["aadt_value_numeric"].gt(0)].copy()
    if valid.empty:
        return pd.DataFrame(columns=["aadt_route_key_v2", "aadt_candidate_record_count"])
    grouped = valid.groupby("aadt_route_key_v2", dropna=False)
    out = grouped.agg(
        aadt_candidate_record_count=("aadt_source_index", "nunique"),
        aadt_candidate_value_count=("aadt_value_numeric", "nunique"),
        aadt_candidate_values=(AADT_VALUE_FIELD, _joined_unique_numbers),
        min_aadt=("aadt_value_numeric", "min"),
        max_aadt=("aadt_value_numeric", "max"),
        latest_aadt_year=("aadt_year_numeric", "max"),
        sample_aadt_route_name_raw=("aadt_route_name_raw", "first"),
    ).reset_index()
    out["route_bucket_type"] = "multi_candidate_route"
    out.loc[out["aadt_candidate_record_count"].eq(1), "route_bucket_type"] = "single_candidate_route"
    return out


def _single_route_bucket_candidates(aadt: gpd.GeoDataFrame, base_bins: gpd.GeoDataFrame, bucket_summary: pd.DataFrame) -> pd.DataFrame:
    single_keys = set(bucket_summary.loc[bucket_summary["aadt_candidate_record_count"].eq(1), "aadt_route_key_v2"].astype(str))
    if not single_keys:
        return pd.DataFrame()
    valid_aadt = aadt.loc[
        aadt["source_geometry_is_valid"]
        & aadt["aadt_route_key_v2"].isin(single_keys)
        & aadt["aadt_value_numeric"].gt(0)
    ].copy()
    valid_aadt = valid_aadt.sort_values(["aadt_route_key_v2", "aadt_source_index"]).drop_duplicates("aadt_route_key_v2")
    routed_bins = base_bins.loc[base_bins["stable_route_key_v2"].astype(str).isin(single_keys)].copy()
    if routed_bins.empty or valid_aadt.empty:
        return pd.DataFrame()
    columns = _aadt_columns_for_matching(valid_aadt)
    out = routed_bins.drop(columns=["geometry"], errors="ignore").merge(
        pd.DataFrame(valid_aadt[columns]),
        left_on="stable_route_key_v2",
        right_on="aadt_route_key_v2",
        how="inner",
    )
    out["candidate_match_family"] = "single_candidate_route_bucket"
    out["route_key_match_status"] = "exact_route_key_match"
    out["nearest_aadt_distance_ft"] = pd.NA
    return out
...
def _route_compatible_candidates(aadt: gpd.GeoDataFrame, base_bins: gpd.GeoDataFrame, *, logger: ProgressLogger | None = None, limit_route_groups: int | None = None, progress_every: int = 25) -> pd.DataFrame:
    valid_aadt = aadt.loc[aadt["source_geometry_is_valid"] & aadt["aadt_route_key_v2"].ne("") & aadt["aadt_value_numeric"].gt(0)].copy()
    routed_bins = base_bins.loc[base_bins["stable_route_key_v2"].astype(str).ne("")].copy()
    columns = _aadt_columns_for_matching(valid_aadt)
    frames: list[pd.DataFrame] = []
    route_groups = list(routed_bins.groupby("stable_route_key_v2", dropna=False))
    ...
    for index, (route_name, base_group) in enumerate(route_groups, start=1):
        aadt_group = valid_aadt.loc[valid_aadt["aadt_route_key_v2"].eq(route_name)].copy()
...
def _nearest_any_aadt_candidates(aadt: gpd.GeoDataFrame, base_bins: gpd.GeoDataFrame, *, logger: ProgressLogger | None = None) -> pd.DataFrame:
    ...
    nearest["route_key_match_status"] = nearest.apply(_route_match_status, axis=1)
...
def _route_match_status(row: pd.Series) -> str:
    stable = str(row.get("stable_route_key_v2") or "")
    aadt = str(row.get("aadt_route_key_v2") or "")
    if not stable:
        return "route_missing"
    if not aadt:
        return "route_missing"
    if stable == aadt:
        return "exact_route_key_match"
...
def _summary_by_source(frame: pd.DataFrame, prefix: str) -> pd.DataFrame:
    ...
    rename = {
        ...
        "aadt_route_key_v2": f"{prefix}_aadt_route_key_v2",
        ...
        "route_key_match_status": f"{prefix}_route_key_match_status",
    }
...
def _build_base_context(base_bins: gpd.GeoDataFrame, single_candidates: pd.DataFrame, route_candidates: pd.DataFrame, route_review: pd.DataFrame, nearest_any: pd.DataFrame, bucket_summary: pd.DataFrame, *, logger: ProgressLogger | None = None) -> pd.DataFrame:
    base = pd.DataFrame(base_bins.drop(columns=["geometry"], errors="ignore")).copy()
    single_summary = _summary_by_source(single_candidates, "single")
    ...
    bucket = bucket_summary[["aadt_route_key_v2", "aadt_candidate_record_count", "route_bucket_type"]].rename(columns={"aadt_route_key_v2": "stable_route_key_v2"})
    work = (
        base.merge(bucket, on="stable_route_key_v2", how="left")
        .merge(single_summary, on="source_bin_key", how="left")
        .merge(route_summary, on="source_bin_key", how="left")
        .merge(review_summary, on="source_bin_key", how="left")
        .merge(any_summary, on="source_bin_key", how="left")
    )
    for column in [
        "route_candidate_count",
        "single_candidate_count",
        "single_nearest_distance",
        "single_aadt_value_conflict_flag",
        "single_selected_aadt_value",
        "single_selected_aadt_year",
        "single_nearest_aadt_record_id",
        "single_aadt_route_name_raw",
        "single_aadt_route_key_v2",
        "single_aadt_edge_rte_key",
        "single_aadt_direction_factor",
        "single_aadt_directionality",
        "single_aadt_candidate_values",
        ...
        "any_aadt_route_key_v2",
        "any_aadt_edge_rte_key",
        "any_aadt_direction_factor",
        "any_aadt_directionality",
        "any_aadt_candidate_values",
        "any_route_key_match_status",
    ]:
        if column not in work.columns:
            work[column] = pd.NA

    stable_route = work["stable_route_key_v2"].fillna("").astype(str)
    single_exists = work["single_candidate_count"].notna()
    ...
    single_conflict = work["single_aadt_value_conflict_flag"].fillna(False).astype(bool)
    route_conflict = work["route_aadt_value_conflict_flag"].fillna(False).astype(bool)
    single_distance = pd.to_numeric(work["single_nearest_distance"], errors="coerce")
    ...
    work["aadt_context_status"] = "missing_no_route_compatible_aadt"
    work["aadt_context_method"] = "no_route_compatible_aadt_match"
    work["aadt_context_confidence"] = "missing"
    work["route_key_match_status"] = "not_evaluated"

    no_route = ~single_exists & ~route_exists
    no_stable_route = no_route & stable_route.eq("")
    any_route_mismatch = no_route & any_exists & work["any_route_key_match_status"].fillna("").astype(str).eq("route_mismatch")
    review_route_only = no_route & review_exists & ~no_stable_route & ~any_route_mismatch
    single_stable = single_exists & ~single_conflict
    route_stable = ~single_exists & route_exists & ~route_conflict
    multi_route_no_measure = no_route & ~stable_route.eq("") & pd.to_numeric(work["aadt_candidate_record_count"], errors="coerce").fillna(0).gt(1)

    work.loc[no_stable_route, ["aadt_context_status", "aadt_context_method", "aadt_context_confidence", "route_key_match_status"]] = ["review_route_missing", "review_route_missing", "low_review", "route_missing"]
    work.loc[any_route_mismatch, ["aadt_context_status", "aadt_context_method", "aadt_context_confidence", "route_key_match_status"]] = ["review_route_mismatch", "review_route_mismatch", "low_review", "route_mismatch"]
    work.loc[route_conflict, ["aadt_context_status", "aadt_context_method", "aadt_context_confidence", "route_key_match_status"]] = ["ambiguous_conflicting_aadt_values", "review_conflicting_aadt_values", "low_review", "exact_route_key_match"]
    work.loc[multi_route_no_measure, ["aadt_context_status", "aadt_context_method", "aadt_context_confidence", "route_key_match_status"]] = ["review_multi_candidate_route_no_measure", "review_multi_candidate_route_no_measure", "low_review", "exact_route_key_match"]
    work.loc[single_stable, ["aadt_context_status", "aadt_context_method", "aadt_context_confidence", "route_key_match_status"]] = ["stable_single_route_candidate", "stable_single_route_candidate", "medium", "exact_route_key_match"]
    work.loc[route_stable, ["aadt_context_status", "aadt_context_method", "aadt_context_confidence", "route_key_match_status"]] = ["stable_route_geometry_disambiguated", "stable_route_geometry_disambiguated", "medium", "exact_route_key_match"]
    work.loc[route_stable & route_distance.le(TIGHT_ROUTE_THRESHOLD_FT), "aadt_context_confidence"] = "high"
    work.loc[review_route_only, "route_key_match_status"] = "exact_route_key_match"

    use_single_fields = single_exists
    use_route_fields = ~single_exists & route_exists
    ...
    for column, value in output_defaults.items():
        work[column] = value
    for output, single_col, route_col, review_col, any_col in [
        ("aadt_route_name_raw", "single_aadt_route_name_raw", "route_aadt_route_name_raw", "review_aadt_route_name_raw", "any_aadt_route_name_raw"),
        ("aadt_route_key_v2", "single_aadt_route_key_v2", "route_aadt_route_key_v2", "review_aadt_route_key_v2", "any_aadt_route_key_v2"),
        ("aadt_edge_rte_key", "single_aadt_edge_rte_key", "route_aadt_edge_rte_key", "review_aadt_edge_rte_key", "any_aadt_edge_rte_key"),
        ("aadt_candidate_values", "single_aadt_candidate_values", "route_aadt_candidate_values", "review_aadt_candidate_values", "any_aadt_candidate_values"),
        ("nearest_aadt_record_id", "single_nearest_aadt_record_id", "route_nearest_aadt_record_id", "review_nearest_aadt_record_id", "any_nearest_aadt_record_id"),
        ("aadt_direction_factor", "single_aadt_direction_factor", "route_aadt_direction_factor", "review_aadt_direction_factor", "any_aadt_direction_factor"),
        ("aadt_directionality", "single_aadt_directionality", "route_aadt_directionality", "review_aadt_directionality", "any_aadt_directionality"),
    ]:
        for mask, source_col in [(use_single_fields, single_col), (use_route_fields, route_col), (use_review_fields, review_col), (use_any_fields, any_col)]:
            if source_col in work.columns:
                work.loc[mask, output] = work.loc[mask, source_col]
    work.loc[use_single_fields, "nearest_aadt_distance_ft"] = single_distance.loc[use_single_fields]
    ...
    work.loc[single_stable, "aadt_value"] = work.loc[single_stable, "single_selected_aadt_value"]
    work.loc[single_stable, "aadt_year"] = work.loc[single_stable, "single_selected_aadt_year"]
    ...
    work["aadt_value_conflict_flag"] = single_conflict | route_conflict
    ...
    work["single_candidate_count"] = pd.to_numeric(work["single_candidate_count"], errors="coerce").fillna(0).astype("int64")
    if logger:
        logger.log(
            ...
            f"missing={int(work['aadt_context_status'].eq('missing_no_route_compatible_aadt').sum())}"
        )
    return work
...
def _build_directional_context(context_bins: pd.DataFrame, base_context: pd.DataFrame) -> pd.DataFrame:
    ...
    for column in ["stable_route_name_raw", "stable_route_key_v2", "stable_edge_key"]:
    ...
    out["aadt_context_status"] = out["aadt_context_status"].fillna("missing_no_route_compatible_aadt")
...
def _reference_signal_summary(bin_context: pd.DataFrame) -> pd.DataFrame:
    ...
    grouped = (
        ...
        .agg(
            ...
            missing_aadt_bin_count=("aadt_context_status", lambda s: int(s.eq("missing_no_route_compatible_aadt").sum())),
...
def _route_match_qa(bin_context: pd.DataFrame) -> pd.DataFrame:
    return (
        bin_context.groupby(["route_key_match_status", "aadt_context_status", "distance_window"], dropna=False)
...
def _summary(bin_context: pd.DataFrame, base_context: pd.DataFrame, crash_context: pd.DataFrame, aadt_joined: pd.DataFrame, paired_qa: pd.DataFrame, source_aadt: gpd.GeoDataFrame) -> pd.DataFrame:
    ...
    rows = [
        ...
        {"metric": "bins_missing_aadt", "value": "", "count": int(bin_context["aadt_context_status"].eq("missing_no_route_compatible_aadt").sum())},
        ...
        {"metric": "single_candidate_route_bucket_recovered_bins", "value": "", "count": int(bin_context["aadt_context_status"].eq("stable_single_route_candidate").sum())},
        {"metric": "multi_candidate_route_no_measure_review_bins", "value": "", "count": int(bin_context["aadt_context_status"].eq("review_multi_candidate_route_no_measure").sum())},
        ...
        {"metric": "bins_with_aadt_by_route_assisted_match", "value": "", "count": int(bin_context["aadt_context_status"].isin(["stable_single_route_candidate", "stable_unique_measure_match", "stable_route_geometry_disambiguated"]).sum())},
    ...
    for window, group in bin_context.groupby("distance_window", dropna=False):
        ...
        rows.append({"metric": "missing_aadt_bins_by_distance_window", "value": str(window), "count": int(group["aadt_context_status"].eq("missing_no_route_compatible_aadt").sum())})
        rows.append({"metric": "single_candidate_route_bucket_recovered_bins_by_distance_window", "value": str(window), "count": int(group["aadt_context_status"].eq("stable_single_route_candidate").sum())})
    for direction, group in bin_context.groupby("signal_relative_direction", dropna=False):
        ...
        rows.append({"metric": "missing_aadt_bins_by_signal_relative_direction", "value": str(direction), "count": int(group["aadt_context_status"].eq("missing_no_route_compatible_aadt").sum())})
...
def _comparison_to_v1(v2_summary: pd.DataFrame) -> pd.DataFrame:
    v1_summary = _read_optional(V1_AADT_SUMMARY_FILE)
    metrics = [
        "bins_with_stable_aadt",
        "route_mismatch_review_bins",
    ...
    for metric in metrics:
        v1_count = _summary_value(v1_summary, metric)
        v2_count = _summary_value(v2_summary, metric)
        rows.append({"metric": metric, "v1_count": v1_count, "v2_count": v2_count, "v2_minus_v1": v2_count - v1_count})
    rows.append(
        {
            "metric": "single_candidate_route_bucket_recovered_bins",
            "v1_count": 0,
            "v2_count": _summary_value(v2_summary, "single_candidate_route_bucket_recovered_bins"),
            "v2_minus_v1": _summary_value(v2_summary, "single_candidate_route_bucket_recovered_bins"),
        }
    )
    return pd.DataFrame(rows)
...
def _findings(summary: pd.DataFrame, outputs: dict[str, Path], *, limit_route_groups: int | None) -> str:
    ...
    lines = [
        "# AADT Context Join v2 Route-Key-First Findings",
        "",
        "## Bounded Question",
        "",
        "Attach AADT as a flagged context layer to the existing 0-2,500 ft directional-bin universe using route-key-first matching adapted from the legacy thirdstep AADT enrichment pattern. Do not alter upstream/downstream interpretation.",
        "",
        "## Run Scope",
        "",
        ...
        f"- route-mismatch review bins: {count('route_mismatch_review_bins')}",
        f"- single-candidate route bucket recovered bins: {count('single_candidate_route_bucket_recovered_bins')}",
        f"- multi-candidate route/no-measure review bins: {count('multi_candidate_route_no_measure_review_bins')}",
        ...
        *[f"- `{path}`" for path in outputs.values()],
        ...
        "",
        "Review route-mismatch and conflicting-value queues before deciding whether this v2 AADT layer should be promoted into the combined directional-bin context table.",
        "",
    ]
    return "\n".join(lines)


def build_aadt_context_join_v2_route_key_first(*, output_root: Path = OUTPUT_ROOT, limit_route_groups: int | None = None, skip_nearest_any: bool = False) -> dict[str, str]:
    started = datetime.now(timezone.utc)
    out_dir = output_root / OUTPUT_DIR
    logger = ProgressLogger(out_dir / "aadt_context_v2_progress.log")
    logger.log(f"START build_aadt_context_join_v2_route_key_first; limit_route_groups={limit_route_groups}; skip_nearest_any={skip_nearest_any}")
    ...
    bucket_summary = _phase(logger, "_route_bucket_summary", _route_bucket_summary, aadt_aliases)
    single_candidates = _phase(logger, "_single_route_bucket_candidates", _single_route_bucket_candidates, aadt_aliases, base_bins, bucket_summary)
    ...
    all_candidates = pd.concat([frame for frame in [single_candidates, route_candidates, route_review, nearest_any] if not frame.empty], ignore_index=True, sort=False)
    base_context = _phase(logger, "_build_base_context", _build_base_context, base_bins, single_candidates, route_candidates, route_review, nearest_any, bucket_summary, logger=logger)
    ...
    aadt_joined = _phase(logger, "_aadt_records_joined", _aadt_records_joined, source_aadt, pd.concat([single_candidates, route_candidates], ignore_index=True, sort=False), base_context)
    ...
    comparison = _comparison_to_v1(summary)
    qa = _context_qa(directional_context, source_aadt, paired_qa, crash_context)
    review_candidates = directional_context.loc[directional_context["aadt_context_status"].isin(["review_route_mismatch", "review_route_missing", "review_multi_candidate_route_no_measure", "review_spatial_fallback_only", "ambiguous_conflicting_aadt_values", "missing_no_route_compatible_aadt"])].copy()
    ambiguous = directional_context.loc[directional_context["aadt_context_status"].eq("ambiguous_conflicting_aadt_values")].copy()
    missing = directional_context.loc[directional_context["aadt_context_status"].eq("missing_no_route_compatible_aadt")].copy()

    outputs = {
        "summary_csv": out_dir / "aadt_context_v2_summary.csv",
        "base_bin_context_csv": out_dir / "base_bin_aadt_context_v2.csv",
        "directional_bin_context_csv": out_dir / "directional_bin_aadt_context_v2.csv",
        "directional_bin_context_0_1000_csv": out_dir / "directional_bin_aadt_context_v2_0_1000ft.csv",
        "directional_bin_context_1000_2500_csv": out_dir / "directional_bin_aadt_context_v2_1000_2500ft.csv",
        "directional_crash_context_csv": out_dir / "directional_crash_aadt_context_v2.csv",
        "reference_signal_summary_csv": out_dir / "reference_signal_aadt_context_summary_v2.csv",
        "route_bucket_summary_csv": out_dir / "aadt_route_bucket_summary_v2.csv",
        "match_candidates_csv": out_dir / "aadt_bin_match_candidates_v2.csv",
        "review_candidates_csv": out_dir / "aadt_bin_review_candidates_v2.csv",
        "ambiguous_matches_csv": out_dir / "aadt_bin_ambiguous_matches_v2.csv",
        "missing_bins_csv": out_dir / "aadt_missing_bins_v2.csv",
        "comparison_to_v1_csv": out_dir / "aadt_context_v2_comparison_to_v1.csv",
        "paired_pseudo_direction_qa_csv": out_dir / "aadt_paired_pseudo_direction_consistency_qa_v2.csv",
        "context_join_qa_csv": out_dir / "aadt_context_v2_qa.csv",
        "findings_md": out_dir / "aadt_context_v2_findings.md",
        "manifest_json": out_dir / "aadt_context_v2_manifest.json",
        "progress_log": out_dir / "aadt_context_v2_progress.log",
    }
    ...
    _write_csv(bucket_summary, outputs["route_bucket_summary_csv"])
    ...
    manifest = {
        ...
        "bounded_question": "read-only AADT route-key-first context refinement for the stable directional-bin universe",
        ...
        "comparison_to_v1": comparison.to_dict(orient="records"),
        "qa": qa.to_dict(orient="records"),
        "outputs": {key: str(path) for key, path in outputs.items()},
    }
    _write_json(manifest, outputs["manifest_json"])
    logger.log("END build_aadt_context_join_v2_route_key_first")
    return {key: str(path) for key, path in outputs.items()}


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Read-only route-key-first AADT v2 context join for directional bins.")
    ...
    outputs = build_aadt_context_join_v2_route_key_first(output_root=args.output_root, limit_route_groups=limit_route_groups, skip_nearest_any=args.skip_nearest_any)
