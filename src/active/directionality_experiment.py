...
def _write_explore_html(
    ...
    try:
        ...
        _copy_output_to_history(html_path, review_history_dir)
...
def _build_output_layout_readme(output_files: dict[str, str], output_dir: Path) -> str:
    ...
    lines = [
        ...
        "",
        "This output folder is organized so active deliverables stay separate from timestamped history copies.",
        "",
        "## Current outputs",
    ]
    ...
    lines.extend(
        [
            "",
            "## History folders",
            "- `tables/history/`, `review/history/`, `review/geojson/history/`, `review/geopackage/history/`, and `runs/history/` preserve timestamped copies from each successful run.",
            "- Files in `current/` are the stable active paths future runs write for downstream handoff.",
...
def run_directionality_experiment() -> int:
    ...
    run_summary_path = runs_current_dir / "run_summary.json"
