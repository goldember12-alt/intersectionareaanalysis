...
def _write_case_study_layers(
    ...
    _copy_output_to_history(outputs["selected_signal_case_study_signals"], review_geojson_history_dir)
    case_crash_layer.to_file(outputs["selected_signal_case_study_high_confidence_crashes"], driver="GeoJSON")
    _copy_output_to_history(outputs["selected_signal_case_study_high_confidence_crashes"], review_geojson_history_dir)
    case_study_area_layer.to_file(outputs["selected_signal_case_study_study_areas"], driver="GeoJSON")
    _copy_output_to_history(outputs["selected_signal_case_study_study_areas"], review_geojson_history_dir)
    case_approach_row_layer.to_file(outputs["selected_signal_case_study_approach_rows"], driver="GeoJSON")
    _copy_output_to_history(outputs["selected_signal_case_study_approach_rows"], review_geojson_history_dir)
...
def _build_output_layout_readme(output_files: dict[str, str], output_dir: Path) -> str:
    ...
    lines.extend(
        [
            ...
            "- Files in `current/` are the stable active paths for downstream handoff.",
...
def run_high_confidence_upstream_downstream_analysis() -> int:
    ...
    metadata_path = runs_current_dir / "high_confidence_analysis_metadata.json"
