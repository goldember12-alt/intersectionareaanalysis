...
def _write_review_geopackage(
    ...
    try:
        ...
        for layer_name, frame in layers:
            ...
            first_layer = False
        _copy_output_to_history(gpkg_path, review_history_dir)
...
def _write_review_geojson_layers(
    ...
    for layer_name, frame in (
        ...
        _copy_output_to_history(path, review_history_dir)
...
def _build_signal_summary(classifications: pd.DataFrame) -> pd.DataFrame:
    ...
    def _single_non_null(values: pd.Series, label: str, study_area_id: object) -> object:
        non_null = [value for value in values.tolist() if pd.notna(value)]
        unique_non_null: list[object] = []
        for value in non_null:
            if value not in unique_non_null:
                unique_non_null.append(value)
        if len(unique_non_null) > 1:
            raise ValueError(
                f"signal summary source has conflicting non-null {label} values for StudyAreaID {study_area_id}: {unique_non_null}"
            )
        return unique_non_null[0] if unique_non_null else pd.NA
    ...
    artifact_summary = (
        classifications.groupby(
            identifier_fields + ["FlowDirectionUsed", "FlowProvenanceUsed"],
            dropna=False,
        )
        .agg(
    ...
    duplicate_sizes = artifact_summary.groupby("StudyAreaID", dropna=False).size()
    invalid_duplicate_sizes = duplicate_sizes.loc[duplicate_sizes.gt(1) & ~duplicate_sizes.eq(2)]
    if not invalid_duplicate_sizes.empty:
        raise ValueError(
            "signal summary artifact has unexpected StudyAreaID multiplicity before collapse: "
            + ", ".join(f"{study_area_id}={count}" for study_area_id, count in invalid_duplicate_sizes.items())
        )

    conflicting_ids: list[str] = []
    for study_area_id, group in artifact_summary.groupby("StudyAreaID", dropna=False):
        if int(group[identifier_fields].nunique(dropna=False).max()) > 1:
            conflicting_ids.append(str(study_area_id))
        _single_non_null(group["FlowDirectionUsed"], "FlowDirectionUsed", study_area_id)
        _single_non_null(group["FlowProvenanceUsed"], "FlowProvenanceUsed", study_area_id)
    if conflicting_ids:
        raise ValueError(
            "signal summary artifact has conflicting identifier values within StudyAreaID groups: "
            + ", ".join(conflicting_ids)
        )

    summary = (
        artifact_summary.groupby(identifier_fields, dropna=False, as_index=False)
        .agg(
    ...
    duplicate_summary_ids = summary.loc[summary.duplicated(subset=["StudyAreaID"], keep=False), "StudyAreaID"].drop_duplicates().tolist()
    if duplicate_summary_ids:
        raise ValueError(
            "signal_study_area_summary__approach_shaped.csv would emit duplicate StudyAreaID rows after summary collapse: "
            + ", ".join(str(value) for value in duplicate_summary_ids)
        )
    return summary
...
def _build_output_layout_readme(output_files: dict[str, str], output_dir: Path) -> str:
    ...
    lines = [
        ...
        "",
        "This output folder is organized so active prototype deliverables stay separate from timestamped history copies.",
        "",
        "## Current outputs",
    ]
    ...
    lines.extend(
        [
            ...
            "- Files in `current/` are the active stable paths the prototype writes for downstream handoff.",
            "- If a `current/` artifact is locked, the corresponding `history/` copy remains the retained run artifact until `current/` is manually cleared and rerun.",
        ]
    )
    return "\n".join(lines) + "\n"
...
def run_upstream_downstream_prototype() -> int:
    ...
    output_files["run_summary"] = str(runs_current_dir / "run_summary.json")
